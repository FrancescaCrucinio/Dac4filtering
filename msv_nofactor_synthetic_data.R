...
for(i in 2:Time.step){
  ...
  x[i, ] <- mu_i + mvrnorm(n = 1, rep(0, d), SigmaU - SigmaUV %*% solve(SigmaV) %*% SigmaUV)
  cov_matrix <- diag(sqrt(exp(x[i-1, ]))) %*% SigmaV %*% diag(sqrt(exp(x[i-1, ])))
  y[i-1, ] <- mvrnorm(n = 1, rep(0, d), cov_matrix)
}
