...
for (t in 1:Time.step){
  res_dac <- marginal_dac_spatial(history, spatial_data$y[, , t], sigmaX, nu, tau, adaptive = TRUE)
  history <- res_dac
  print(paste(t))
}
toc()
boxplot(c((apply(res_dac, c(1,2), mean) - spatial_data$x[, , Time.step+1])^2), log="y")
