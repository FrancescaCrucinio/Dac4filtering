...
for (id in 1:50){
  filename <- paste0("data/lgssm_results/lgssm_d32N5000ID", id, sep = "")
  dfnew <- read.csv(filename)
  dfnew$N <- "5*10^3"
  dfnew$d <- "32"
  dfnew$run <- id
...
rmse_data <- data.frame(rep(1:Time.step, times = 18), rep(tmp$algo, each = 100), rep(tmp$N, each = 100), rep(tmp$d, each = 100))
...
ggplot(data = rmse_data, aes(x = Time.step, y = rmse, group = algo, colour = algo)) +
  ...
  facet_wrap(~factor(interaction(N, d), levels=c("10^2.32", "5*10^2.32", "10^3.32", "5*10^3.32",
                                                 "10^2.256", "5*10^2.256", "10^3.256", "5*10^3.256",
                                                 "10^2.2048", "5*10^2.2048", "10^3.2048")), ncol = 4, nrow = 3) +
