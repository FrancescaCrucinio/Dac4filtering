...
for (i in 1:11){
  ...
  df <- read.csv(paste0("/storage/u1693998/results/dac_spatial_d", d, "N", Nparticles, "ID", i, "step", 1), row.names = 1,
...
for (i in 12:50){
  ground_truth <- unname(data.matrix(read.csv(paste0("/storage/u1693998/data/data_truth_spatial_tau_", -tau, "d", d, "ID", i),
                                              row.names = 1, nrows=d, skip=(Time.step)*d)))
  df1 <- read.csv(paste0("/storage/u1693998/results/means_dac_spatial_d", d, "N", Nparticles, "ID", i, "step", 1), row.names = 1,
                 nrows=d, skip=(5-1)*d)
  df2 <- read.csv(paste0("/storage/u1693998/results/means_dac_spatial_d", d, "N", Nparticles, "ID", i, "step", 2), row.names = 1,
                  nrows=d, skip=(5-1)*d)
  data_dac[, , i] <- unname(data.matrix(df2[, 1:d]))

  dac_time[i] <- df1[1, d+2] + df2[1, d+2]
...
write.csv(x=df_dac, file=paste0("run_dac/results/stats_dac_spatial_tau", -tau, "d", d, "N", Nparticles))
