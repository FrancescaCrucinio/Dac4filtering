...
filename <- paste0("/storage/u1693998/results/dac_lgssm_d", d, "N", Nparticles, "ID", ID, "step", timeinterval)
write.table(res_dac, file = filename, append = FALSE, sep = " ", dec = ".",
            row.names = TRUE, col.names = TRUE)
# mix
if(timeinterval > 1){
  filename <- paste0("/storage/u1693998/results/mix_dac_lgssm_d", d, "N", Nparticles, "ID", ID, "timeinterval", timeinterval-1)
  history[, , 2] <- matrix(0, ncol = d, nrow = Nparticles)
  history[, , 1] <- unname(data.matrix(read.table(filename, row.names = 1)))
}
tic()
res_dac_mix <- dac_lgssm_crossover(history, y[t, ], tau, lambda, sigmaY)
runtime <- toc()
...
# light
if(timeinterval > 1){
  filename <- paste0("/storage/u1693998/results/light_dac_lgssm_d", d, "N", Nparticles, "ID", ID, "timeinterval", timeinterval-1)
  history[, , 2] <- matrix(0, ncol = d, nrow = Nparticles)
  history[, , 1] <- unname(data.matrix(read.table(filename, row.names = 1)))
}
tic()
...
filename <- paste0("/storage/u1693998/results/light_dac_lgssm_d", d, "N", Nparticles, "ID", ID, "step", timeinterval)
write.table(res_dac_light, file = filename, append = FALSE, sep = " ", dec = ".",
            row.names = TRUE, col.names = TRUE)
# adaptive light
if(timeinterval > 1){
  filename <- paste0("/storage/u1693998/results/v2dac_lgssm_d", d, "N", Nparticles, "ID", ID, "timeinterval", timeinterval-1)
  history[, , 2] <- matrix(0, ncol = d, nrow = Nparticles)
  history[, , 1] <- unname(data.matrix(read.table(filename, row.names = 1)))
}
tic()
res_dac_v2 <- dac_lgssm_lightweight_crossover(history, y[t, ], tau, lambda, sigmaY, "adaptive")
runtime <- toc()
m_dac_v2 <- colMeans(res_dac_v2)
df <- data.frame(rbind(df, t(c(m_dac_v2, runtime))))
filename <- paste0("/storage/u1693998/results/v2_dac_lgssm_d", d, "N", Nparticles, "ID", ID, "step", timeinterval)
write.table(res_dac_v2, file = filename, append = FALSE, sep = " ", dec = ".",
            row.names = TRUE, col.names = TRUE)

df$algo <- as.factor(c("dac", "mix", "light", "light_ada_temp"))
df$mutation <- as.factor(c(1,1,1,1))
