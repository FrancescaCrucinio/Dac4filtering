...
for (t in ti_begin:ti_end) {
  # dac (lightweight)
  tic()
  res_dac <- marginal_dac_lgssm_lightweight(history, y[t-ti_begin+1, ], tau, lambda, sigmaY, adaptive = TRUE)
  history <- res_dac
  rt <- toc()
  ...
  if(t == ti_end){
    filename <- paste0("/storage/u1693998/results/adaptive_dac_lgssm_d", d, "N", Nparticles, "ID", ID, "timeinterval", timeinterval)
    write.table(res_dac, file = filename, append = FALSE, sep = " ", dec = ".",
                row.names = TRUE, col.names = TRUE)
  }
}

filename <- paste0("/storage/u1693998/results/adaptive_marginal_dac_lgssm_d", d, "N", Nparticles, "ID", ID, "timeinterval", timeinterval)
