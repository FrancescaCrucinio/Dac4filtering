...
write.csv(x=df_nsmc, file=paste0("run_dac/results/stats_nsmc_nl_iid_d", d, "N", Nparticles, "ID", ID))

df_stpf <- data.frame(rbind(mse_stpf, var_stpf))
df_stpf$type <- rep(c("mse", "var"), each = d)
df_stpf$runtime <- mean(stpf_time)
write.csv(x=df_stpf, file=paste0("run_dac/results/stats_stpf_nl_iid_d", d, "N", Nparticles, "ID", ID))
