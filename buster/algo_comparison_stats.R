...
df <- data.frame()
data_light <- read.csv(paste0("run_dac/results/light_lgssm_d", d, "N", Nparticles, "ID", ID), row.names = 1)
data_nsmc <- read.csv(paste0("run_dac/results/nsmc_lgssm_d", d, "N", Nparticles, "ID", ID), row.names = 1)

rse_dac_light <- (unname(data.matrix(data_light[1:Time.step, 1:d])) - true_means)^2/true_variances
rse_nsmc <- (unname(data.matrix(data_nsmc[1:Time.step, 1:d]))  - true_means)^2/true_variances
...
df <- data.frame(rbind(df, cbind(t(rse_dac_light), res_dac_light_w1, res_dac_light_ks, rep(data_light$runtime, times = d))))
df <- data.frame(rbind(df, cbind(t(rse_nsmc), res_nsmc_w1, res_nsmc_ks, rep(data_nsmc$runtime, times = d))))
