  ...
  for (u in 1:nlevels){
    ...
    for (i in 1:nodes){
      ...
      for(j in ci[1]:ci[2]){
        ...
        indices <- mult_resample(W[, j]/sum(W[, j]), m*Nparticles)
        # apply random permutation
        indices <- sample(indices)
        xTmp[, j] <- x[indices, j]
        xOldTmp[, j] <- xOld[indices, j]
        max.lW <- max(lW[, j])
        lZNew[i] <- lZNew[i] + log(mean(W[, j])) + max.lW
      }
      # mixture weights
      ...
      max.lWmix <- max(lWmix)
      Wmix <- exp(lWmix - max.lWmix)
      lZNew[i] <- lZNew[i] + log(mean(Wmix)) + max.lWmix -
        0.5*Sigma.det[[u]][nchild*(i-1)+1] - 0.5*Sigma.det[[u]][i*nchild] + 0.5*Sigma.det[[u+1]][i]
      # resampling the new population
      indices <- mult_resample(Wmix/sum(Wmix), Nparticles)
      for(n in 1:Nparticles) {
        # update particles
        xNew[n, ci[1]:ci[2]] <- c(xTmp[indices[n], ci[1]:(ci[1]+nv-1)], xTmp[indices[n], (ci[1]+nv):ci[2]])
        # update xOld
        xOldNew[n, ci[1]:ci[2]] <- c(xOldTmp[indices[n], ci[1]:(ci[1]+nv-1)], xOldTmp[indices[n], (ci[1]+nv):ci[2]])
        }
      }
    x <- xNew
    xOld <- xOldNew
