# Tempering for linear Gaussian SSM
marginal_lgssm_tempering <- function(ci, i, nv, lambda, tau, sigmaY, obs, x, xOld, historyIndex, historyIndexNew, after_mix_lW, ess_target, ess_decay_threshold, mcmc_sd){
  # tree topology
  nchild <- 2
  ...
  while (current_alpha < 1 & !same_sign) {
    ...
    if(ess < Nparticles / 2){
      ...
      historyIndexNew <- historyIndexNew[resampled_indices, ]
  ...
  return(list("x" = x, "history_index_updated" = historyIndex, "lWmix" = after_mix_lW))
}

# RWM with crossover operators for linear Gaussian SSM
