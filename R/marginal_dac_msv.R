...
marginal_dac_msv <- function(history, obs_current, obs_past, SigmaV, SigmaUV, SigmaX, phi, adaptive = FALSE){
  # dimension
  d <- ncol(history)
  # number of particles
  ...
  for (i in 1:d){
    ...
    mu <- sapply(sample_from_past, function(s){phi*history[s, ] + SigmaUV %*% solve(SigmaV) %*% diag(1/sqrt(exp(history[s, ]))) %*% obs_past})
    x[, i] <- mu[i, ] + sqrt(SigmaX[i, i])*rnorm(Nparticles)
