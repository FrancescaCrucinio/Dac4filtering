...
dac_time_lgssm_crossover <- function(tau, lambda, sigmaY, Nparticles, x0, y, method = "adaptive", M = NULL, marginals = NULL){
  ...
  for (t in 1:Time.step) {
    switch(method,
           lc={
             # linear cost dac
             res_dac <- dac_lgssm_lc_crossover(x0, y[t, ], tau, lambda, sigmaY)
           },
           mix={
             # dac with mixture weights
             res_dac <- dac_lgssm_crossover(x0, y[t, ], tau, lambda, sigmaY)
           }, # dac with lightweight mixture weights (no adaptation)
           light={
             res_dac <- dac_lgssm_lightweight_crossover(x0, y[t, ], tau, lambda, sigmaY)
           }, # dac with adaptive lightweight mixture weights
           {
             res_dac <- dac_lgssm_lightweight_crossover(x0, y[t, ], tau, lambda, sigmaY, "adaptive")
           }
    )
    x0 <- res_dac[, 1:d]
    m[t, ] <- colMeans(x0)
  ...
  if(!is.null(marginals)){
    # compare marginals at last time step
    ks_dac <- apply(rbind(x0, marginals), ks_dist, N = Nparticles, MARGIN = 2)
    w1_dac <- apply(rbind(x0, marginals), w1_dist, N = Nparticles, MARGIN = 2)
  }
  else{
    ks_dac <- NULL
    w1_dac <- NULL
