...
marginal_msv_light_fixed_theta_first_step_vectorized <- function(i, u, nv, ci, W, Nparticles, theta, SigmaV, Sigma0, x, obs){
  ...
  obs_node <- apply(obs_covariance, 3, function(x) {-0.5*obs[ci[1]:ci[2]] %*% solve(x[ci[1]:ci[2], ci[1]:ci[2]]) %*% obs[ci[1]:ci[2]]})
  obs_left <- apply(obs_covariance, 3, function(x) {-0.5*obs[ci[1]:(ci[1]+nv-1)] %*% solve(x[ci[1]:(ci[1]+nv-1), ci[1]:(ci[1]+nv-1)]) %*% obs[ci[1]:(ci[1]+nv-1)]})
  obs_right <- apply(obs_covariance, 3, function(x) {-0.5*obs[(ci[1]+nv):ci[2]] %*% solve(x[(ci[1]+nv):ci[2], (ci[1]+nv):ci[2]]) %*% obs[(ci[1]+nv):ci[2]]})
