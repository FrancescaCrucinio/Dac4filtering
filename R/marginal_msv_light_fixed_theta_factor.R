...
marginal_msv_light_fixed_theta_factor <- function(i, u, nv, ci, W, Nparticles, theta, mu, Phi, Sigma, Lambda, x, history){
  ...
  for (n in 1:(theta*Nparticles)) {
    ...
    obs_node <- -0.5*t(obs[ci[1]:p]) %*% solve(obs_covariance) %*% obs[ci[1]:p]
    obs_left <- -0.5*t(obs[ci[1]:(ci[1]+nv-1)]) %*% diag(exp(1/mx[ci[1]:(ci[1]+nv-1)])) %*% obs[ci[1]:(ci[1]+nv-1)]
    obs_right <- -0.5*t(obs[ci[1]:p]) %*% solve(obs_covariance_right) %*% obs[ci[1]:p]
