...
marginal_msv_light_fixed_theta_nofactor <- function(i, u, nv, ci, W, Nparticles, theta, mu, Phi, Sigma, x, history){
  ...
  for (n in 1:(theta*Nparticles)) {
    ...
    transition_node <- mean(exp(-0.5*rowSums(sweep(all_nodes_means, 2, mx[ci[1]:ci[2]])^2 %*% diag(1/Sigma[ci[1]:ci[2]]^2))))
    transition_left <- mean(exp(-0.5*rowSums(sweep(all_nodes_means_left, 2, mx[ci[1]:(ci[1]+nv-1)])^2 %*% diag(1/Sigma[ci[1]:(ci[1]+nv-1)]^2, nrow = nv))))
    transition_right <- mean(exp(-0.5*rowSums(sweep(all_nodes_means_right, 2, mx[(ci[1]+nv):ci[2]])^2 %*% diag(1/Sigma[(ci[1]+nv):ci[2]]^2, nrow = length((ci[1]+nv):ci[2])))))
...
marginal_msv_light_fixed_theta_nofactor_vectorized <- function(i, u, nv, ci, W, Nparticles, theta, mu, Phi, Sigma, x, history){
  ...
  # mixture weights
  integral_per_dimension <- array(0, dim = c(theta*Nparticles, Nparticles, ndimensions))
  for (j in 1:nv) {
    dimension <- (ci[1]:ci[2])[j]
    integral_per_dimension[, , j] <- outer(x[indices1, dimension], all_nodes_means[, j], "-")^2/(Sigma[dimension]^2)
  }
  for (j in (nv+1):ndimensions) {
    dimension <- (ci[1]:ci[2])[j]
    integral_per_dimension[, , j] <- outer(x[indices2, dimension], all_nodes_means[, j], "-")^2/(Sigma[dimension]^2)
  }
  ...
  integral_right <- rowMeans(exp(-0.5*rowSums(integral_per_dimension[, , (nv+1):ndimensions, drop = FALSE], dims = 2)))
