car_tempering_crossover <- function(ci, i, nv, lambda, tau, sigmaY, obs, x, xOld, historyIndex, historyIndexNew, after_mix_lW, ess_target, ess_decay_threshold, mcmc_sd){
  # tree topology
  nchild <- 2
  ...
  while (current_alpha < 1 & !same_sign) {
    ...
    updated_particles <- car_mcmc_move(x, ci, i, nv, sigmaY, tau, lambda, xOldv, xOld, historyIndex, obs, after_mix_lW, mcmc_sd, new_alpha)
...
car_mcmc_move <- function(x, ci, i, nv, sigmaY, tau, lambda, xOldv, xOld, historyIndex, obs, after_mix_lW, mcmc_sd, new_alpha){
  Nparticles <- nrow(x)
  nchild <- 2
  propose_x <- x[, ci[1]:ci[2]] + mcmc_sd*matrix(rnorm(Nparticles*length(ci[1]:ci[2])), nrow = Nparticles)
  ...
  for (n in 1:Nparticles) {
    ...
    r1[n] <- -0.5/sigmaX * sum((propose_x[n, ci[1]:(ci[1]+nv-1)] - (cumsum(x[n, ci[1]:(ci[1]+nv-1)][seq(length.out = (nv-1))] + cumsum(rev(left_ancestor)))/d))^2
                      + (x[n, ci[1]:(ci[1]+nv-1)] - (cumsum(x[n, ci[1]:(ci[1]+nv-1)][seq(length.out = (nv-1))] + cumsum(rev(left_ancestor)))/d))^2) -
      0.5/sigmaX * sum((propose_x[n, (ci[1]+nv):ci[2]] - (cumsum(x[n, (ci[1]+nv):ci[2]][seq(length.out = (nv-1))] + cumsum(rev(right_ancestor)))/d))^2
