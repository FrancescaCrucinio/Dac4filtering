  ...
  for (u in 1:nlevels){
    ...
    for (i in 1:nodes) {
      ...
      for (j in 1:nodes) {
        ...
        #   # tempering
        #   tempering_out_top <- nl_tempering(Nparticles, 1-1e-05, xNew, history, historyIndex[, , , 2*i-1, 2*j-1], historyIndex[, , , 2*i-1, 2*j],
        ...
        #   lW_right <- lW[2*i, 2*j, ]
        #   # tempering
        #   tempering_out_bottom <- nl_tempering(Nparticles, 1-1e-05, xNew, history, historyIndex[, , , 2*i, 2*j-1], historyIndex[, , , 2*i, 2*j],
        ...
        # merge
        out_merge <- nl_merge(lW, obs, x, history, historyIndex, 2*i-1, 2*i, 2*j-1, 2*j-1, cir[, 1], c(cic),
                              cir[, 2], c(cic), nv, nvNew, list("u" = u, "direction" = "v"), M, covariance, tau)
        x[c(cir), c(cic), ] <- out_merge$x
        historyIndex[, , , i, j] <- historyIndexNew[out_merge$indices[, 1], , ]
        # if(tempering){
        #   # tempering
        ...
        #   # update history
        #   historyIndex[, , , i, j] <- tempering_out_merge$history_index_updated
        # }
      }
    }
...
dac_nl_adaptive_lightweight <- function(history, obs, sigmaX, nu, M = NULL,
                               covariance = FALSE, tempering = FALSE, obs_old = NULL, tau = NULL){
  # dimension and number of particles
  d <- nrow(history)
  Nparticles <- dim(history)[3]
