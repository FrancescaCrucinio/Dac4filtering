dac_time_lgssm <- function(tau, lambda, sigmaY, Nparticles, x0, y, method = "light", M = NULL){
  ...
  for (t in 1:Time.step) {
    switch(method,
           lc={
             # linear cost dac
             res_dac <- dac_lgssm_lc(xOld, y[t, ], tau, lambda, sigmaY, Sigma.det)
           },
           mix={
             # dac with mixture weights
             res_dac <- dac_lgssm(xOld, y[t, ], tau, lambda, sigmaY, Sigma.det)
           }, # dac with lightweight mixture weights
           {
             res_dac <- dac_lgssm_lightweight(xOld, y[t, ], tau, lambda, sigmaY, Sigma.det, M)
           }
    )
    xOld <- res_dac[, 1:d]
    lZ[t] <- res_dac[1, d+1]
